...
from IMAGE_QUADRUPLE_SEPARATE import split_images,re_concate_the_splitted_images
def eval_generic(original_path=None, sliced_path=None, dest_path=None, para=None):
    """
    Arguments loading
    """
    parser = ArgumentParser()
    # parser.add_argument('--model', default='/root/data/Model_saves_lvlanjiejing/Mar17_22.26.17_retrain_s03/Mar17_22.26.17_retrain_s03_1500.pth')
    # parser.add_argument('--data_path',default='/root/data/videoanno/self_dataset/')
    # parser.add_argument('--output', default='/root/data/videoanno/self_dataset/results')
    # parser.add_argument('--top', type=int, default=5)
    #the parser.top is for the top k objects to be propagated 
    # parser.add_argument('--amp_off', action='store_true')
    # parser.add_argument('--mem_every', default=5, type=int)

    parser.add_argument('--model', default=para.load_network)
    parser.add_argument('--data_path',default=para.data_path)
    parser.add_argument('--output', default=para.output)
    parser.add_argument('--top', default=int(para.top))
    # the parser.top is for the top k objects to be propagated 
    parser.add_argument('--amp_off', action='store_true')
    parser.add_argument('--mem_every', default=int(para.mem_every))
    parser.add_argument('--include_last', help='include last frame as temporary memory?', action='store_true')
    parser.add_argument('--separate_flag', default=para.separate_flag)
    args = parser.parse_args()
    

    test_folder_path = os.path.join(args.data_path, 'test-dev')
    if args.separate_flag==True:
        split_images(test_folder_path)
    out_path = args.output
    args.amp = not args.amp_off

    # Simple setup
    if args.separate_flag==True:
        out_path=os.path.join(out_path, 'sliced')
        #暂时将sliced_output存在/results/sliced,后续再将其拼接到/results
    os.makedirs(out_path, exist_ok=True)

    torch.autograd.set_grad_enabled(False)
    #print("ok1")
    # Setup Dataset
    test_dataset = GenericTestDataset(data_root=test_folder_path,separate_flag=args.separate_flag)
    ...
    # 拼接results/sliced中的图片至results文件夹
    #original_path="/root/data/videoanno/self_dataset/trainval/Annotations"
    #dest_path="/root/data/videoanno/self_dataset/results"
    if args.separate_flag==True:
        re_concate_the_splitted_images(original_path,sliced_path,dest_path)
    
    
    # #对比color
    # color_confrontation(args.data_path)

...
class GenericTestDataset(Dataset):
    def __init__(self, data_root,separate_flag, res=480):
        if separate_flag==True:
            self.image_dir = path.join(data_root, 'JPEGImages/sliced')
            self.mask_dir = path.join(data_root, 'Annotations/sliced')
        else:
        ...
        # Pre-reading
        for vid in vid_list:
            if (separate_flag==False and vid != "sliced") or separate_flag==True:
    ...
    def __getitem__(self, idx):
        ...
        for i, f in enumerate(frames):
            ...
            #mask_file = path.join(vid_gt_path)
